...
function SortBy(paginationConfig: PaginateConfig<any>) {
    ...
    .map(([fieldName, filterOperations]) => {
      const operations =
        filterOperations === true || filterOperations === undefined
          ? [...Object.values(FilterComparator), ...Object.values(FilterSuffix), ...Object.values(FilterOperator)]
      ...
      return ApiQuery({
        ...
        description: `Filter by ${fieldName} query param.
          ...
          ${p('Example', `filter.${fieldName}=$not:$ilike:John Doe&filter.${fieldName}=$ilike:John`)}
...
export const ApiPaginationQuery = (paginationConfig: PaginateConfig<any>) => {
  return applyDecorators(
    ...[Page(), Limit(paginationConfig), Where(paginationConfig), SortBy(paginationConfig), Search(paginationConfig), SearchBy(paginationConfig), Select(paginationConfig)].filter(
      (v): v is MethodDecorator => v !== undefined,
    ),
  );
};
