...
import { ArrayContains, Between, Brackets, Equal, FindOperator, ILike, In, IsNull, LessThan, LessThanOrEqual, MoreThan, MoreThanOrEqual, Not, SelectQueryBuilder } from 'typeorm';
import { WherePredicateOperator } from 'typeorm/query-builder/WhereClause';
import { PaginateQuery, SelectOneQuery } from './decorator';
import { checkIsArray, checkIsEmbedded, checkIsRelation, extractVirtualProperty, fixColumnAlias, getPropertiesByColumnName, isISODate } from './helper';
...
export const isOperator = (value: unknown): value is FilterOperator => values(FilterOperator).includes(value as any);

export enum FilterSuffix {
  NOT = '$not',
}

export const isSuffix = (value: unknown): value is FilterSuffix => values(FilterSuffix).includes(value as any);
...
export const isComparator = (value: unknown): value is FilterComparator => values(FilterComparator).includes(value as any);

export const OperatorSymbolToFunction = new Map<FilterOperator | FilterSuffix, (...args: any[]) => FindOperator<string>>([
...
export const generatePredicateCondition = (qb: SelectQueryBuilder<unknown>, column: string, filter: Filter, alias: string, isVirtualProperty = false): WherePredicateOperator =>
  qb['getWherePredicateCondition'](isVirtualProperty ? column : alias, filter.findOperator) as WherePredicateOperator;

export const addWhereCondition = <T>(qb: SelectQueryBuilder<T>, column: string, filter: ColumnsFilters) => {
...
export const parseFilterToken = (raw?: string): FilterToken | null => {
  // console.log('raw', raw);
  ...
  if (notValue.length) {
    token.value = token.operator === FilterOperator.NULL ? undefined : raw.replace(`${notValue.join(OPERAND_SEPARATOR)}${OPERAND_SEPARATOR}`, '');
  }

  return token;
};

export const parseFilter = (query: PaginateQuery | SelectOneQuery, filterableColumns?: { [column: string]: (FilterOperator | FilterSuffix)[] | true }): ColumnsFilters => {
  ...
  for (const column of Object.keys(query.filter)) {
    ...
    for (const raw of statements) {
      const _raw = raw as string;
      const token = parseFilterToken(_raw);
      if (!token) {
        continue;
      }
...
export const addFilter = <T>(
  ...
  query: PaginateQuery | SelectOneQuery,
