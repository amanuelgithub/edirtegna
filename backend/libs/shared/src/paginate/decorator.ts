...
const isRecord = (data: unknown): data is Record<string, unknown> => data !== null && typeof data === 'object' && !Array.isArray(data);

const isExpressRequest = (request: unknown): request is ExpressRequest => isRecord(request) && typeof request.get === 'function';
...
export interface SelectOneQuery {
  searchBy?: string[];
  search?: string;
  filter?: { [column: string]: number | string | string[] };
  select?: string[];
}
...
const parseParam = <T>(queryParam: unknown, parserLogic: (param: string, res: any[]) => void): T[] | undefined => {
  const res = [];
  if (queryParam) {
...
export const Paginate = createParamDecorator((_data: unknown, ctx: ExecutionContext): PaginateQuery => {
  ...
  const filter = mapKeys(
    pickBy(query, (param, name) => name.includes('filter.') && (isString(param) || (Array.isArray(param) && (param as any[]).every((p) => isString(p))))) as Dictionary<
      string | string[]
    >,
