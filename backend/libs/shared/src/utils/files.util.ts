...
export class FilesUtil {
  static getEnvPath(envDir?: string): string | string[] {
    const projectRoot = process.cwd();
    const env: string | undefined = process.env.NODE_ENV || 'production';
    const envDefault: string = resolve(projectRoot, `.env`);
    const envFilePath: string = resolve(projectRoot, `${envDir ? envDir + '/' : ''}${env}.env`);
    const result: string[] = [];
    const defaultEnvExist = existsSync(envDefault);
    const envFileExist = existsSync(envFilePath);
    if (!defaultEnvExist && !envFileExist) {
      throw new HttpException(`Environment File(s) Not found at : ${envDefault} and / or ${envFilePath}`, HttpStatus.NOT_FOUND);
    }
    if (defaultEnvExist) {
      result.push(envDefault);
    }
    if (envFileExist) {
  ...
  //     }
  //   }
  static deleteFile = async (path: string): Promise<boolean> => {
    Logger.log(`Unlinking file ${path}`);
    const filePath = resolve(process.cwd(), `${path}`);
    // rewrite using fs.promises
    const exists = await fsp
      .access(filePath, constants.F_OK)
      .then(() => true)
      .catch(() => false);
    if (!exists) {
      Logger.warn(`Unlink failed. File ${filePath} not found`);
      return false;
    }
    // unlink file
    await fsp.unlink(filePath).catch((e) => {
      Logger.error(`Unlink failed. File ${filePath} not deleted`, e);
      return false;
    });
    Logger.log(`File ${filePath} deleted`);
    return true;
  };
  static async removeEmptyDirectories(directory: string, level = 0) {
    ...
    if (fileNames.length > 0) {
      const recursiveRemovalPromises = fileNames.map((fileName) => this.removeEmptyDirectories(path.join(directory, fileName), level + 1));
  ...
  //       const maxSizeBytes = typeof fileProps.maxSize === 'number' ? fileProps.maxSize : StringUtil.fileSizeStringToBytes(fileProps.maxSize);
  ...
  //   }
  //   static createFilesResponseDto<T>(files: Record<string, Express.Multer.File[]>, data: any): Record<string, string | string[]> {
  ...
  //   private static replacePatternVariables(pattern: string, meta?: FileMetadata, extraVariables: Record<string, any> = {}): string {
  //     const now = new Date();
  //     const variables = {
