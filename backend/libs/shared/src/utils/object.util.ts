export class ObjectUtil {
  static transfromKeysAndValues(
    ...
    newValue: (key: string, value: any, keyList?: string[]) => any | { key: string; value: any }[] = (key, value) => value,
    mergeKeyValue: (key: string, value: any, newKey: string, newValue: any, keyList?: string[]) => { key: string; value: any; remove?: boolean } = (
      key,
      value,
      newKey,
      newValue,
    ...
    for (const key of Object.keys(object)) {
      ...
      if (newKeyResult) {
        if (Object.keys(acc).includes(newKeyResult)) {
          const { key: resolvedKey, value: resolvedValue, remove } = mergeKeyValue(key, acc[newKeyResult], newKeyResult, newValueResult, keyList);
          if (remove) {
        ...
        for (const newResult of newValueResult) {
          ...
          if (Object.keys(acc).includes(newResult.key)) {
            const { key: resolvedKey, value: resolvedValue, remove } = mergeKeyValue(key, acc[newResult.key], newResult.key, newResult.value, keyList);
            if (remove) {
  ...
  static async transfromKeysAndValuesAsync(
    ...
    newValue: (key: string, value: any, keyList?: string[]) => Promise<any | { key: string; value: any }[]> = async (key, value) => value,
    mergeKeyValue: (key: string, value: any, newKey: string, newValue: any, keyList?: string[]) => Promise<{ key: string; value: any; remove?: boolean }> = async (
      key,
      value,
      newKey,
      newValue,
    ...
    for (const key of Object.keys(object)) {
      ...
      if (newKeyResult) {
        if (Object.keys(acc).includes(newKeyResult)) {
          const { key: resolvedKey, value: resolvedValue, remove } = await mergeKeyValue(key, acc[newKeyResult], newKeyResult, newValueResult, keyList);
          if (remove) {
            delete acc[newKeyResult];
          }
          acc[resolvedKey] = resolvedValue;
        ...
        for (const newResult of newValueResult) {
          ...
          if (Object.keys(acc).includes(newResult.key)) {
            const { key: resolvedKey, value: resolvedValue, remove } = await mergeKeyValue(key, acc[newResult.key], newResult.key, newResult.value, keyList);
            if (remove) {
              delete acc[newResult.key];
            }
            acc[resolvedKey] = resolvedValue;
  ...
  static pruneNullAndUndefined(obj: any): any {
    ...
    if (Array.isArray(obj)) {
      const cleanedArray = obj.map(ObjectUtil.pruneNullAndUndefined).filter((value) => value !== null && value !== undefined);
