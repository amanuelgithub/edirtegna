...
import type { IDelOptions, IMultiCommands, IOptions, IRedisClient, ISetOptions, Transaction } from './jsonCache.types';
...
export default class JSONCache<T = any> implements IJSONCache<T> {
  ...
  public async get(key: string, ...fields: string[]): Promise<Partial<T> | undefined> {
    const [data, typeInfo] = await Promise.all([this.redisClientInt.hgetall(this.getKey(key)), this.redisClientInt.hgetall(this.getTypeKey(key))]);
  ...
  public async clearAll(): Promise<any> {
    ...
    do {
      [cursor, keys] = await this.redisClientInt.scan(cursor, 'MATCH', `${this.options.prefix}*`, 'COUNT', Config.SCAN_COUNT);
